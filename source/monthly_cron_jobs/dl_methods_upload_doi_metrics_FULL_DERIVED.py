...
def grow_derived_set(copied_to_lookup_dict, master_set, last_iteration_set, copy_only):
...
#            next_iteration_set.add(copied_to_lookup_dict[object_ws_obj_id])
            next_iteration_set = next_iteration_set | set(copied_to_lookup_dict[object_ws_obj_id])
        # swap and replace "/" with ":" for query against RE for input objects.
...
#        print("master set post append: " + str(master_set))
        master_set = grow_derived_set(copied_to_lookup_dict, master_set, next_iteration_set, copy_only)
...
#TRYING NEW WAY
            #print("ws_objects_to_track :: " + str(ws_objects_to_track))
            for ws_object_to_track in ws_objects_to_track:
                all_copied_only_objects_from_ws_object = grow_derived_set(copied_to_lookup_dict, set([ws_object_to_track]), set([ws_object_to_track]),1)
...
#                all_derived_objects_from_ws_object = list()
                all_derived_objects_from_ws_object = grow_derived_set(copied_to_lookup_dict, set([ws_object_to_track]), set([ws_object_to_track]),0)
