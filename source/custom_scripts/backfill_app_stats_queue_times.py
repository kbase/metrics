...
#    return 1;
    ...
    update_queue_times_without_job_id = "update metrics.user_app_usage "\
                                        "set queue_time = %s "\
    ...
    update_queue_times_with_job_id = "update metrics.user_app_usage set queue_time = %s where job_id = %s ;";
    ...
    for record in app_usage_list:
        ...
        if "job_id" not in record:
            input = [round((record['exec_start_time'] - record['creation_time'])),
                     record['user_id'], 
            ...
            input = [round((record['exec_start_time'] - record['creation_time'])), record.get('job_id')]
