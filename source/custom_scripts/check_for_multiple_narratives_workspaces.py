...
#{"ws":{"$gt":56000},
ws_obj_vers_cursor = db.workspaceObjVersions.find({"type":{"$in":["KBaseNarrative.Narrative-1.0",
...
#    print(str(ws_obj_ver))
    ...
    if ws_obj_ver["id"] not in ws_narratives_dict[ws_obj_ver["ws"]]:
        ws_narratives_dict[ws_obj_ver["ws"]][ws_obj_ver["id"]] = {"max_version":1, "savedate":None}
    if ws_obj_ver["ver"] > ws_narratives_dict[ws_obj_ver["ws"]][ws_obj_ver["id"]]["max_version"]:
        ws_narratives_dict[ws_obj_ver["ws"]][ws_obj_ver["id"]]["max_version"] = ws_obj_ver["ver"]

multiple_narratives_ws_count = 0
for ws in sorted(ws_narratives_dict.keys()):
    if len(ws_narratives_dict[ws]) > 1:
        print("WS " + str(ws) + " has " + str(len(ws_narratives_dict[ws])) + " different narratives")
        multiple_narratives_ws_count += 1
        print("Workspace: " + str(ws) + " : " + str(ws_narratives_dict[ws]))
print("TOTAL NUMBER OF WORKSPACES WITH MULTIPLE NARRATIVES " + str(multiple_narratives_ws_count))
