...
# GetEE2AppStats
ee2 = execution_engine2(url='https://kbase.us/services/ee2', token=os.environ['METRICS_USER_TOKEN'])
...
#    exit(0)
    ...
    user_app_insert_statement = "insert into metrics.user_app_usage_ee2 " \
...
#                                "values(%s,%s,%s,FROM_UNIXTIME(%s),FROM_UNIXTIME(%s),%s,%s,%s,%s,%s);"
    ...
    check_dup_no_job_id_statement = "select count(*) from metrics.user_app_usage_ee2 " \
...
#                                    "and finish_date = FROM_UNIXTIME(%s) " \
    ...
    check_dup_no_job_id_no_app_name_statement = "select count(*) from metrics.user_app_usage_ee2 " \
                                                "where job_id is NULL " \
                                                "and username = %s " \
...
#            is_error = True
        input = [record.get('job_id'),record['user'], 
#                 helper_concatenation(record["app_module_name"], record["app_id"]),
                 record['app_name'],
                 record['start_date'], record['finish_date'],
                 round(record['run_time']),
                 round((record['queue_time'])),
                 record['is_error'], record['git_commit_hash'], 
